...
module adder_1bit_0 ( a, b, carry_in, sum, carry_out );
  ...
  INVX2 U1 ( .A(n7), .Y(carry_out) );
  XOR2X1 U2 ( .A(carry_in), .B(n8), .Y(sum) );
  AOI22X1 U3 ( .A(b), .B(a), .C(n8), .D(carry_in), .Y(n7) );
  XOR2X1 U4 ( .A(a), .B(b), .Y(n8) );
endmodule
...
module adder_16bit ( a, b, carry_in, sum, overflow );
  ...
  adder_nbit_BIT_WIDTH16 nb ( .a(a), .b(b), .carry_in(carry_in), .sum(sum), 
        .overflow(overflow) );
endmodule
